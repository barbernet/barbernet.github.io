/**
 * BarberFlow Pro - صفحة توثيق الصالون
 * المسار: onboarding/verification/salon.js
 * 
 * ⚠️ ملاحظة: هذا الكود يفترض وجود جدول `verifications` في Supabase
 * سنضيف الجدول لاحقاً عند الانتهاء من التصميم
 */
...
import { safeExecute, handleSupabaseError } from '../../shared/utils/error-handler.js';
...
const ownershipUpload = document.getElementById('ownershipUpload');
const ownershipPlaceholder = document.getElementById('ownershipPlaceholder');
const ownershipPreview = document.getElementById('ownershipPreview');
const ownershipFileName = document.getElementById('ownershipFileName');
const ownershipFileSize = document.getElementById('ownershipFileSize');
const removeOwnership = document.getElementById('removeOwnership');
...
async function initializePage() {
    ...
    if (business.is_verified) {
        showStatusAlert('صالونك موثق بالفعل! لا حاجة لتقديم طلب جديد.', 'success');
        setTimeout(() => {
    ...
    salonNameInput.value = business.name || '';
...
function validateCurrentStep() {
    switch (currentStep) {
        ...
        case 4:
            const ownershipType = document.querySelector('input[name="ownershipType"]:checked');
            if (!ownershipType) {
                showNotification('يرجى اختيار نوع وثيقة الملكية', 'error');
                return false;
            }
            if (!ownershipFile) {
                showNotification('يرجى رفع وثيقة الملكية', 'error');
                return false;
            }
...
// رفع وثيقة الملكية
// ============================================
ownershipUpload.addEventListener('click', () => {
    if (!ownershipFile) ownershipInput.click();
});

ownershipInput.addEventListener('change', async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    ...
    ownershipFileName.textContent = file.name;
    ownershipFileSize.textContent = `${(file.size / 1024).toFixed(2)} KB`;
    ownershipPreview.style.display = 'block';
    ownershipPlaceholder.style.display = 'none';
});

removeOwnership.addEventListener('click', (e) => {
    e.stopPropagation();
    ownershipFile = null;
    ownershipPath = null;
    ownershipPreview.style.display = 'none';
    ownershipPlaceholder.style.display = 'flex';
...
form.addEventListener('submit', async (e) => {
    ...
    const result = await safeExecute(async () => {
        ...
        // 3. رفع وثيقة الملكية
        const ownershipResult = await uploadImage(ownershipFile, `verifications/${businessData.id}`, {
            prefix: 'ownership'
        });
        
        if (!ownershipResult.success) throw new Error(ownershipResult.error);
        ownershipPath = ownershipResult.path;

        // 4. حفظ بيانات التوثيق في قاعدة البيانات
        ...
        const { data: verification, error: verificationError } = await supabase
            ...
            .insert({
                ...
                rib: ribClean,
                ownership_doc_url: ownershipPath,
...
skipBtn.addEventListener('click', () => {
    showNotification('يمكنك تقديم طلب التوثيق لاحقاً من إعدادات الصالون', 'info');
    setTimeout(() => {
        window.location.replace(resolvePath('PROFILE_SALON'));
    }, 1500);
});
