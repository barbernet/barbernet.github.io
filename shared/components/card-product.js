/**
BarberFlow Pro - مكون بطاقة المنتج
المسار: shared/components/card-product.js
الدور: إنشاء وعرض بطاقات المنتجات بشكل احترافي
⚠️ متوافق مع جدول products في Supabase (guide.md)
*/
...
<article class="store-card">
  ...
  <div class="store-card__content">
    ...
    <div class="store-card__rating">
      ...
      <span class="store-card__rating-value">0.0</span>
      <span class="store-card__rating-count">(0)</span>
    </div>
...
/**
جلب متوسط تقييم المنتج وعدد التقييمات من جدول reviews
...
*/
export async function createProductCard(product, id) {
  const productId = id || product?.id;
  if (!productId) {
    console.error('[ProductCard] ❌ المعرف (id) غير مُعرّف!');
    return null;
  }

  const parser = new DOMParser();
  const doc = parser.parseFromString(PRODUCT_CARD_TEMPLATE, 'text/html');
  ...
  try {
    ...
    // ===== التقييم (جلب من جدول reviews) ✅ تحديث مهم =====
    const ratingElement = card.querySelector('.store-card__rating-value');
    const ratingCountElement = card.querySelector('.store-card__rating-count');
    const starsContainer = card.querySelector('.store-card__stars');

    const { rating, count: reviewsCount } = await fetchProductRating(productId);
    ...
    if (ratingCountElement) {
      ratingCountElement.textContent = `(${reviewsCount})`;
    }
    ...
    if (priceCurrent) {
      ...
      priceCurrent.textContent = `${price.toFixed(2)} DH`;
    }

    if (product.old_price) {
      const oldPrice = parseFloat(product.old_price);
      if (priceOld) {
        priceOld.textContent = `${oldPrice.toFixed(2)} DH`;
    ...
    console.error("[ProductCard] Critical Processing Error:", error);
    return null;
  }
}
...
*/
export async function createProductCards(products) {
  const cards = [];
  for (const product of products) {
    const card = await createProductCard(product, product.id);
